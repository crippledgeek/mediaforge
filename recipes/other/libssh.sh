...
if [ -f "$PREFIX/lib/libmbedcrypto.a" ]; then
  ...
  PKG_CMAKE_FLAGS="-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DWITH_MBEDTLS=ON -DWITH_GCRYPT=OFF -DWITH_SERVER=OFF -DWITH_EXAMPLES=OFF -DUNIT_TESTING=OFF -DMBEDTLS_ROOT_DIR=$PREFIX"
else
  PKG_DISABLED=true
  PKG_FFMPEG_OPT=""
