...
Install reconciles against the manifest of the previous install rather than
replacing it. Every file the earlier build put in the prefix and this one does
not ship — a library that was renamed, merged into another archive, or dropped
...
installed every previous entry would look like an orphan; it says so rather
than reporting a successful install of zero files.
