...
if [ "$_n" != 3 ]; then
  _bad one-privileged-exec-per-file "three files cost $_n privileged execs, want 3 (one each): $(printf '%s' "$_out" | tail -1)"
elif [ "$(cat "$_d/lib/a" 2>/dev/null)" != "PAYLOAD-a" ] \
  || [ "$(cat "$_d/lib/b" 2>/dev/null)" != "PAYLOAD-b" ] \
  || [ "$(cat "$_d/lib/c" 2>/dev/null)" != "PAYLOAD-c" ]; then
  _bad one-privileged-exec-per-file "the one-exec-per-file path did not place all three payloads under $_d/lib"
else
...
if [ -e "$_esc/b" ]; then
  _bad swapped-directory-refused-next-file "a file was written THROUGH the symlink swapped in mid-run, to $_esc"
elif ! printf '%s' "$_out" | grep -q 'Refusing a privileged write'; then
  _bad swapped-directory-refused-next-file "the swapped directory was not refused: $(printf '%s' "$_out" | tail -2)"
elif [ "$_n" != 2 ]; then
  _bad swapped-directory-refused-next-file "the swap case cost $_n privileged execs, want 2 (one per file)"
elif [ "$(cat "$_d/lib.orig/a" 2>/dev/null)" != "PAYLOAD-a" ]; then
  _bad swapped-directory-refused-next-file "the file installed before the swap never landed — the refusal proves nothing"
else
...
if [ -e "$_esc/sub" ]; then
  _bad class-dir-refused-before-mkdir "mkdir created $_esc/sub THROUGH the symlinked include/ before anything refused it"
  ...
  _bad class-dir-refused-before-mkdir "the symlinked include/ was not refused: $(printf '%s' "$_out" | tail -2)"
elif [ "$_n" != 2 ]; then
  _bad class-dir-refused-before-mkdir "one landing plus one refusal cost $_n privileged execs, want 2"
...
if [ "$_n" != 1 ]; then
  _bad unprivileged-file-spends-no-exec "one unprivileged and one privileged file cost $_n privileged execs, want 1: $(printf '%s' "$_out" | tail -1)"
elif [ "$(cat "$_d/lib/a" 2>/dev/null)" != "PAYLOAD-a" ] \
  || [ "$(cat "$_d/lib/b" 2>/dev/null)" != "PAYLOAD-b" ]; then
  _bad unprivileged-file-spends-no-exec "the mixed-privilege run did not place both payloads under $_d/lib"
else
...
if [ -f "$_d/lib/a" ]; then
  _bad empty-helper-caught-at-read "an empty helper still placed $_d/lib/a — it cannot have; something else wrote it"
elif [ -f "$_d/.manifest" ] && grep -q 'lib/a' "$_d/.manifest" 2>/dev/null; then
  _bad empty-helper-caught-at-read "an empty helper produced a manifest entry for a file it never wrote"
elif ! printf '%s' "$_out" | grep -q 'is empty'; then
  _bad empty-helper-caught-at-read "an empty helper was not diagnosed at read time: $(printf '%s' "$_out" | tail -2)"
else
...
if [ -f "$_d/lib/a" ]; then
  _bad noop-helper-refused-not-believed "a no-op helper still placed $_d/lib/a — it cannot have; something else wrote it"
elif [ -f "$_d/.manifest" ] && grep -q 'lib/a' "$_d/.manifest" 2>/dev/null; then
  _bad noop-helper-refused-not-believed "a no-op helper produced a manifest entry for a file it never wrote"
elif ! printf '%s' "$_out" | grep -q 'INSTALLED sentinel'; then
  _bad noop-helper-refused-not-believed "a helper that exited 0 without working was believed: $(printf '%s' "$_out" | tail -2)"
else
...
if [ -f "$_d/lib/a" ]; then
  ...
  _bad damaged-helper-diagnosed-as-damage "a damaged helper was reported as a symlink attack — the operator is sent after nothing"
elif ! printf '%s' "$_out" | grep -q 'failed to parse'; then
  _bad damaged-helper-diagnosed-as-damage "a helper that cannot parse was not diagnosed: $(printf '%s' "$_out" | tail -2)"
else
...
if [ -f "$_d/lib/a" ]; then
  _bad refused-sudo-names-policy-and-way-out "the install proceeded even though sudo refused to run the helper"
elif printf '%s' "$_out" | grep -q 'cannot resolve the install destination'; then
  _bad refused-sudo-names-policy-and-way-out "a refused sudo was diagnosed as an unresolvable destination — the wrong fix entirely"
elif ! printf '%s' "$_out" | grep -q 'could not run the install helper'; then
  _bad refused-sudo-names-policy-and-way-out "a refused sudo was not diagnosed: $(printf '%s' "$_out" | tail -3)"
elif ! printf '%s' "$_out" | grep -q 'sudoers'; then
  _bad refused-sudo-names-policy-and-way-out "the refusal was reported without naming the sudoers policy that causes it"
elif ! printf '%s' "$_out" | grep -q 'as root'; then
  _bad refused-sudo-names-policy-and-way-out "the refusal named the cause but not the way out — installing as root needs no per-file sudo"
else
