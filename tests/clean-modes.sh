...
# makepkg(8) has no option that touches SRCDEST at all. The GNU standards put the
# fetched distribution below the floor of even distclean ("should leave only the
# files that were in the distribution"), and say the one target that deletes
# hard-to-rebuild things SHOULD start by announcing itself — a convention, not a
# mandate, and what `all-says-what-it-discards-before-discarding-it` holds --all
# to.
#
