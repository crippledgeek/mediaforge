...
_wired flag-documented mediaforge.sh '--allow-tmpfs'
...
if [ ! -f lib/storage.sh ]; then
  for _a in refuses-ram-disk names-the-override allows-ram-disk-when-asked \
            ...
            unreadable-free-space-does-not-refuse dry-run-is-exempt; do
...
case "$_floor" in
  '' | *[!0-9]*) _bad floor-is-readable "MF_MIN_FREE_KB is not a plain number in lib/storage.sh"; _floor=41943040 ;;
esac
...
_probe=$(mktemp -d) || { printf 'FAIL [tmpdir]\n' >&2; exit 1; }
case "$(mf_fs_type "$_probe")" in
  tmpfs | ramfs | devtmpfs)
    _dry=$( cd "$_probe" && "$ROOT/mediaforge.sh" build --dry-run --yes 2>&1 ) || true
    case "$_dry" in
      *RAM-backed*) _bad dry-run-is-exempt "a dry run was refused, and it writes nothing" ;;
      *) _pass dry-run-is-exempt ;;
    esac
    ;;
  *)
