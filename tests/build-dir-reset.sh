...
#
# Before this, twenty sites reset a build directory themselves -- thirteen as
# `rm -rf build && mkdir -p build`, four as the same pair on two lines, one over
# three sibling directories with the rm's stderr sent to /dev/null, and two as a
# bare removal the build system then recreated. Not one of them looked at either
# status. Nothing in mediaforge sets `set -e`, so a failed `rm -rf` short-circuits
...
if [ "$_sites" -ge 18 ]; then
  _pass recipes-route-through-the-helper
else
  _bad recipes-route-through-the-helper "only $_sites recipe(s) call mf_reset_dir; expected >=18"
fi
...
for _f in $(find recipes -name '*.sh' | sort); do
  _own="$_own$(_code_only "$_f" | awk -v F="$_f" '
...
: > "$_ok/CMakeCache.txt"
_reasons=""
mf_reset_dir "$_ok" || _reasons=" helper returned non-zero"
...
_reasons=""
mf_reset_dir "$_multi/8bit" "$_multi/10bit" "$_multi/12bit" || _reasons=" helper returned non-zero"
